function sheetThree(workbook, inputDataOne, inputDataTwo) {
  var sheet = workbook.worksheets().add("Sheet3");
  var tNTwo = 4;
  ...
  for (let i = 0; i < inputDataOne.length; i++) {
    if (inputDataOne[i].employee !== null) {
      sheet.getCell("A3").value("【" + inputDataOne[i].employee + "】");
      var mergedOne = sheet
        .mergedCellsRegions()
        .add(4 + count, 0, 4 + count, 1);
      ...
      var mergedTwo = sheet
        .mergedCellsRegions()
        .add(4 + count, 2, 4 + count, 3);
      mergedTwo.value(sum);
      var mergedThree = sheet
        .mergedCellsRegions()
        .add(4 + count, 4, 4 + count, 5);
      ...
      var mergedFour = sheet
        .mergedCellsRegions()
        .add(4 + count, 7, 4 + count, 9);
      mergedFour.value(inputDataOne[i].code);

      count++;
    } else {
      swapNumber.push(inputDataOne[i]);
    }
  }
  ...
  if (swapNumber.length !== 0) {
    sheetFour(workbook, swapNumber, inputDataTwo);
  }
}
